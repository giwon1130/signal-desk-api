...
class MarketOverviewService(
    ...
    fun getOverview(): MarketOverviewResponse {
        ...
        val watchAlerts = buildWatchAlerts(
            core = core,
            news = news,
            watchlist = watchlist,
            portfolio = portfolio,
        ...
        val briefing = buildBriefing(core.briefing, watchAlerts)
    ...
    fun getSummary(): MarketSummaryResponse {
        val core = getCoreSnapshot()
        val workspaceSnapshot = buildWorkspaceSnapshot(loadKoreanQuotes())
        val watchAlerts = buildWatchAlerts(
            core = core,
            news = getCachedNews().news,
            watchlist = workspaceSnapshot.watchlist,
            portfolio = workspaceSnapshot.portfolio,
            aiRecommendations = workspaceSnapshot.aiRecommendations,
        )
        val briefing = buildBriefing(core.briefing, watchAlerts)
    ...
    private fun buildWorkspaceSnapshot(quotes: Map<String, StockQuote>): WorkspaceSnapshot {
        val watchlist = refreshWatchlist(
            baseWatchlist() + workspaceStore.loadWatchlist().map { item ->
                WatchItem(
                    id = item.id,
                    market = item.market,
                    ticker = item.ticker,
        ...
        val portfolio = refreshPortfolio(
            mergePortfolio(
                basePortfolio(),
                workspaceStore.loadPortfolioPositions().map { position ->
                    HoldingPosition(
                        id = position.id,
                        market = position.market,
                        ticker = position.ticker,
    ...
    private fun buildWatchAlerts(
        core: CachedMarketCore,
        ...
        val hotSignals = core.alternativeSignals.filter { it.score >= 75 }.map { it.label }
    ...
    private fun buildBriefing(base: DailyBriefing, watchAlerts: List<WatchAlert>): DailyBriefing {
    ...
    private fun alertSeverityRank(severity: String): Int {
    ...
    private fun formatSignedRate(value: Double): String = if (value > 0) "+${"%.2f".format(value)}%" else "${"%.2f".format(value)}%"
