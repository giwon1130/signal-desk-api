...
class NewsDigestScheduler(
    ...
    fun runKrClose() {
        ...
        if (!marketSessionService.isKrTradingDay(today)) {
            log.debug("KR news digest skipped — non-trading day {}", today)
            return
        }
    ...
    fun runUsClose() {
        val usTradingDate = LocalDate.now(ZoneId.of("Asia/Seoul")).minusDays(1)
        if (!marketSessionService.isUsTradingDay(usTradingDate)) {
            log.debug("US news digest skipped — non-trading day {}", usTradingDate)
            return
        }
